...
function mimicServerCall(url = "http://mimicServer.example.com", config = {}) { //makes server request 
  return new Promise(function (resolve, reject) {
    setTimeout(function () {
      let isRandomFailure = Math.random() < .2
      if (isRandomFailure) {
        reject("Random server error. Try again."); // REJECTED 
      } else {
        resolve("Pretend remote server notified of action!"); // FULFILLED
      }
    }, 300);
  });
}
